...
// ★ NEW v3.1.2: Delete evidence row by caseId + url match (superadmin only)
function deleteEvidence(caseId, url) {
  try {
    const user = getCurrentUser();
    if (user.role !== 'superadmin') return { ok:false, message:'Hanya superadmin yang dapat menghapus evidence.' };
    const sh = _ss().getSheetByName(SH_EVIDENCE);
    if (!sh || sh.getLastRow() < 2) return { ok:false, message:'Evidence tidak ditemukan.' };
    const rows = sh.getDataRange().getValues();
    for (let i = rows.length - 1; i >= 1; i--) {
      if (String(rows[i][EV.CASE_ID-1]) === String(caseId) && String(rows[i][EV.URL-1]) === String(url)) {
        sh.deleteRow(i + 1);
        _log(caseId, 'DELETE_EVIDENCE', '', '', url, '', _displayName(user));
        return { ok:true, message:'Evidence dihapus.' };
      }
    }
    return { ok:false, message:'Evidence tidak ditemukan.' };
  } catch(e) { return { ok:false, message:e.message }; }
}

// ★ NEW v3.1.2: Upload file (base64) → save to Drive → record URL in EVIDENCE sheet
// Client sends: base64-encoded file content + mimeType + filename
// Server saves to Drive folder, sets share link, stores URL in sheet
function addEvidenceFile(caseId, base64Data, mimeType, filename, description) {
  try {
    const user = getCurrentUser();
    if (!base64Data || !filename) return { ok:false, message:'Data file tidak lengkap.' };
    ...
    const type    = _mimeCategory(mimeType);
    ...
    return { ok:true, message:'File berhasil diupload.', url, fileId, filename };
  } catch(e) {
    return { ok:false, message:'Upload gagal: ' + e.message };
  }
}

// Get or auto-create the evidence folder in Drive
function _getEvidenceFolder() {
  const props    = PropertiesService.getScriptProperties();
  const savedId  = props.getProperty('EVIDENCE_FOLDER_ID');
  if (savedId) {
    try { return DriveApp.getFolderById(savedId); } catch(e) { /* folder deleted or no access */ }
  }
  // Auto-create "AFP Evidence Files" at Drive root
  const folder = DriveApp.createFolder('AFP Evidence Files');
  props.setProperty('EVIDENCE_FOLDER_ID', folder.getId());
  return folder;
}

// Admin: set a custom Drive folder ID for evidence storage
// Run this once from Apps Script editor: setEvidenceFolderById('YOUR_FOLDER_ID')
function setEvidenceFolderById(folderId) {
  try {
    const folder = DriveApp.getFolderById(folderId); // validates access
    PropertiesService.getScriptProperties().setProperty('EVIDENCE_FOLDER_ID', folderId);
    return { ok:true, message:'Evidence folder set: ' + folder.getName() + ' (' + folderId + ')' };
  } catch(e) {
    return { ok:false, message:'Folder tidak ditemukan atau tidak ada akses: ' + e.message };
  }
}

// Get current evidence folder info (for admin verification)
function getEvidenceFolderInfo() {
  try {
    const id = PropertiesService.getScriptProperties().getProperty('EVIDENCE_FOLDER_ID');
    if (!id) return { ok:true, data:{ id:null, name:'Belum diset — akan dibuat otomatis saat upload pertama.' } };
    const folder = DriveApp.getFolderById(id);
    return { ok:true, data:{ id, name:folder.getName(), url:'https://drive.google.com/drive/folders/'+id } };
  } catch(e) {
    return { ok:false, message:e.message };
  }
}

// Map MIME type to evidence category
function _mimeCategory(mimeType) {
  if (!mimeType) return 'doc';
  const m = String(mimeType).toLowerCase();
  if (m.startsWith('image/'))                          return 'image';
  if (m === 'application/pdf')                         return 'pdf';
  if (m.includes('word') || m.includes('document'))   return 'doc';
  if (m.includes('sheet') || m.includes('excel'))     return 'doc';
  return 'doc';
}
...
// ★ v3.1.2: Run ONCE from Apps Script editor (▶ Run button) after deploying
// to grant Drive permission. Error "Anda tidak memiliki izin memanggil DriveApp"
// means this function hasn't been run yet to authorize the Drive scope.
function authorizeDrive() {
  try {
    const root = DriveApp.getRootFolder();
    // Also verify / create the evidence folder now so first upload is instant
    const folder = _getEvidenceFolder();
    return {
      ok: true,
      message: '✅ Drive berhasil diotorisasi. Evidence folder: "' + folder.getName() + '" (ID: ' + folder.getId() + ')'
    };
  } catch(e) {
    return { ok:false, message:e.message };
  }
